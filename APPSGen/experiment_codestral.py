...
class codestral_experiment:
    def __init__(self, temperature) -> None:
        ...
        if not os.path.exists("APPSGen/data_codestral/"+self.time):
            os.makedirs("APPSGen/data_codestral/"+self.time)
        return   
    ...
    def run_persona(self,start,size):
        ...
        for i in range(start, start + size):
            ...
            # 产生代码的prompt
            with open ('APPSGen/data_codestral/persona.jsonl', 'a') as f:
            ...
            with open('APPSGen/data_codestral/persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_codestral/persona.jsonl', 'APPSGen/data_codestral/'+ self.time +'/persona_'+self.time+'.jsonl')
        os.rename('APPSGen/data_codestral/persona_result.jsonl', 'APPSGen/data_codestral/'+ self.time +'/persona_result_'+self.time+'.jsonl')
    ...
    def run_compare(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_codestral/compare_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_codestral/compare_result.jsonl', 'APPSGen/data_codestral/'+ self.time +'/compare_result_'+self.time+'.jsonl')
    ...
    def run_common_persona(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_codestral/common_persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_codestral/common_persona_result.jsonl', 'APPSGen/data_codestral/'+ self.time +'/common_persona_result_'+self.time+'.jsonl')
