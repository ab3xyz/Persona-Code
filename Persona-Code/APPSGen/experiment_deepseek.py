...
class deepseek_experiment:
    def __init__(self, temperature) -> None:
        ...
        if not os.path.exists("APPSGen/data_deepseek/"+self.time):
            os.makedirs("APPSGen/data_deepseek/"+self.time)
        return   
    ...
    def run_persona(self,start,size):
        ...
        for i in range(start, start + size):
            ...
            # 产生代码的prompt
            with open ('APPSGen/data_deepseek/persona.jsonl', 'a') as f:
            ...
            with open('APPSGen/data_deepseek/persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_deepseek/persona.jsonl', 'APPSGen/data_deepseek/'+ self.time +'/persona_'+self.time+'.jsonl')
        os.rename('APPSGen/data_deepseek/persona_result.jsonl', 'APPSGen/data_deepseek/'+ self.time +'/persona_result_'+self.time+'.jsonl')
    ...
    def run_compare(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_deepseek/compare_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_deepseek/compare_result.jsonl', 'APPSGen/data_deepseek/'+ self.time +'/compare_result_'+self.time+'.jsonl')
    ...
    def run_common_persona(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_deepseek/common_persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_deepseek/common_persona_result.jsonl', 'APPSGen/data_deepseek/'+ self.time +'/common_persona_result_'+self.time+'.jsonl')


if __name__ == "__main__":
    ee = deepseek_experiment(0.1)
    ee.run_common_persona(85,500-85)
