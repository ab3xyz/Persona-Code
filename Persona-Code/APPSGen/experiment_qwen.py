...
class qwen_experiment:
    def __init__(self, temperature) -> None:
        ...
        if not os.path.exists("APPSGen/data_qwen/"+self.time):
            os.makedirs("APPSGen/data_qwen/"+self.time)
        return   
    ...
    def run_persona(self,start,size):
        ...
        for i in range(start, start + size):
            ...
            # 产生代码的prompt
            with open ('APPSGen/data_qwen/persona.jsonl', 'a') as f:
            ...
            with open('APPSGen/data_qwen/persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_qwen/persona.jsonl', 'APPSGen/data_qwen/'+ self.time +'/persona_'+self.time+'.jsonl')
        os.rename('APPSGen/data_qwen/persona_result.jsonl', 'APPSGen/data_qwen/'+ self.time +'/persona_result_'+self.time+'.jsonl')
    ...
    def run_compare(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_qwen/compare_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_qwen/compare_result.jsonl', 'APPSGen/data_qwen/'+ self.time +'/compare_result_'+self.time+'.jsonl')
    ...
    def run_common_persona(self, start, size):
        ...
        for i in range(start, start + size):
            ...
            result = dict(solution=generated_code)
            with open('APPSGen/data_qwen/common_persona_result.jsonl', 'a') as f:
                json_str = json.dumps(result)
                f.write(json_str+'\n')
        os.rename('APPSGen/data_qwen/common_persona_result.jsonl', 'APPSGen/data_qwen/'+ self.time +'/common_persona_result_'+self.time+'.jsonl')
